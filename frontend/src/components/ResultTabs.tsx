    ...
    <div className="answer-card">
      <div className="answer-top">
        <div>
          ...
          {result.cached && result.cache_layer !== 'L3' && (
            <p className="answer-cached">
              本次未查库：答案来自应答缓存，是首跑那一次的完整结果。
              下方耗时与成本因此为 0，数据新鲜度以「数据快照」为准。
            </p>
          )}
          {/* L3 与上面那句**不能共用一段话**：它真的查了库。
              把"本次未查库"挂在一份刚刚实时查出来的结果上，是一句假话，
              而且方向最坏 —— 用户会以为这个数是旧的，转头去手动重跑一遍。
              这一层省掉的是模型，不是查询。 */}
          {result.cached && result.cache_layer === 'L3' && (
            <p className="answer-cached">
              本次结果为实时重跑：复用了此前同类提问生成的查询语句，
              重新过护栏执行了一次，<strong>数字是当下的</strong>；
              结论措辞沿用那一次，未调用模型，因此成本为 0。
            </p>
          )}
