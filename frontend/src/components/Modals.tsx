...
export const hasTaskFilter = (filters: TaskFilters) =>
  filters.source !== 'all' || filters.risk !== 'all' || filters.user !== 'all' || filters.since !== 'all'

export interface TaskFilterOption { value: string; label: string }

export const TASK_SINCE_LABEL: Record<string, string> = {
  all: '全部时间',
  today: '今天',
  '7d': '近 7 天',
  '30d': '近 30 天',
...
export function TaskFilterModal({ value, sources, users, onClose, onApply }: {
  value: TaskFilters
  sources: TaskFilterOption[]
  users: TaskFilterOption[]
  onClose: () => void
  onApply: (filters: TaskFilters) => void
}) {
  /* 草稿态：改选项不立刻生效，点「应用筛选」才回写。
     边选边筛的话，列表在弹窗背后跳，而弹窗盖住的正是要看的那片列表。 */
  const [draft, setDraft] = useState<TaskFilters>(value)
  const set = (patch: Partial<TaskFilters>) => setDraft(current => ({ ...current, ...patch }))

  const summary = [
    draft.source !== 'all' ? `数据源 ${sources.find(item => item.value === draft.source)?.label ?? draft.source}` : '',
    draft.risk !== 'all' ? `风险 ${draft.risk}` : '',
    draft.user !== 'all' ? `发起人 ${users.find(item => item.value === draft.user)?.label ?? draft.user}` : '',
    draft.since !== 'all' ? `发起时间 ${TASK_SINCE_LABEL[draft.since] ?? draft.since}` : '',
  ].filter(Boolean).join(' · ')

  return (
    <div className="modal task-filter-modal" role="dialog" aria-modal="true" aria-labelledby="taskFilterTitle">
      <div className="modal-head">
        <div>
          <div className="eyebrow">TASK FILTER</div>
          <h3 id="taskFilterTitle">筛选查询任务</h3>
          <p>组合筛选仅影响当前列表，不会修改任务状态。</p>
        </div>
        <button className="modal-close" type="button" onClick={onClose} aria-label="关闭筛选">×</button>
      </div>
      <div className="modal-body">
        <div className="form-grid">
          <div className="form-row">
            <label htmlFor="taskFilterSource">数据源</label>
            <select id="taskFilterSource" value={draft.source} onChange={event => set({ source: event.target.value })}>
              <option value="all">全部数据源</option>
              {sources.map(item => <option key={item.value} value={item.value}>{item.label}</option>)}
            </select>
          </div>
          {/* 风险档不是审计里的字段，是后端按已记录事实折算的（askdb/audit.py 的
              _risk）。所以这里筛的是折算结果，和任务行上显示的是同一个值。 */}
          <div className="form-row">
            <label htmlFor="taskFilterRisk">风险等级</label>
            <select id="taskFilterRisk" value={draft.risk} onChange={event => set({ risk: event.target.value })}>
              <option value="all">全部风险</option>
              <option value="HIGH">HIGH · 越权或触碰不该碰的数据</option>
              <option value="MEDIUM">MEDIUM · 扫描量或成本偏高</option>
              <option value="LOW">LOW · 未触及任何边界</option>
            </select>
          </div>
          <div className="form-row">
            <label htmlFor="taskFilterUser">发起人</label>
            <select id="taskFilterUser" value={draft.user} onChange={event => set({ user: event.target.value })}>
              <option value="all">全部发起人</option>
              {users.map(item => <option key={item.value} value={item.value}>{item.label}</option>)}
            </select>
          </div>
          <div className="form-row">
            <label htmlFor="taskFilterSince">发起时间</label>
            <select id="taskFilterSince" value={draft.since} onChange={event => set({ since: event.target.value })}>
        ...
        <div className="task-filter-note" aria-live="polite">
          {summary ? `将按「${summary}」收窄当前列表。` : '当前未设置高级筛选，将显示所选状态下的全部任务。'}
        </div>
        <div className="modal-actions">
          <button className="ghost" type="button" onClick={() => setDraft(EMPTY_TASK_FILTERS)}>重置筛选</button>
          <button className="primary" type="button" onClick={() => onApply(draft)}>应用筛选</button>
        </div>
      </div>
    </div>
  )
