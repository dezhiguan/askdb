...
export interface AskResult {
  ...
  /** 召回**降级**过：配置声明 vector，实际跑的是 keyword。与盲选分开显示——
   *  盲选是一张都没命中，降级照样能挑出几张表，答案于是长得完全正常。 */
  ...
  cached_from?: string
  /** 命中的是哪一层缓存。**只有 L2 / L3 会带这个字段**，L1 不带
   *  （L1 是"逐字相同的那一句话"，没有歧义可言）。
   *
   *  L2 与 L3 在页面上必须说不同的话：L2 是原样返回旧结果，L3 是复用上次那条
   *  SQL **实时重跑**了一次 —— 对后者说"本次未查库"是假话，而且会让人以为
   *  这个数是旧的，转头再手动跑一遍。 */
  cache_layer?: 'L2' | 'L3'
  /** 这次命中省下了多少钱 / 多少毫秒（取首跑那次的实付）。
