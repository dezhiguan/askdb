...
规模 64 题：单表聚合 15 · 多表关联 15 · 口径依赖 10 · 时间窗口 5 ·
应被拒绝 7 · 多跳 8 · 安全边界 4。其中 18 题标记为盲测集。

2026-09-13 补的 6 道是安全题（x01-x06）。此前这一层**一道题都没有**：
scene 字段全空、category=security 一条没有，于是 replay 那边的越权率、
敏感数据泄漏率、危险 SQL 拦截率与「安全场景覆盖」整块恒为"无用例"——
一套完整的安全判定摆在那里，从来没有被任何题目触发过。

安全题**一律不进盲测集**，这是有意的：盲测隔离防的是"照着题调参"，
而护栏是确定性的门，不存在过拟合；这几项又必须每一轮都看得见，
藏进盲测集等于平时看不到安全指标。

样例库上**有意不造敏感信息题**。这个库里没有任何个人信息列
（只有文件名、状态、计数），造一道 no_leak 题只会稳定产出一个
漂亮的「泄漏率 0%」—— 那是没考过，不是守住了。这一项在样例库上
就该显示"无用例"，等换到带个人信息列的数据源再补。
