...
9. 按名称定位某一个具体实体时，优先用主键 id 精确匹配；只有在问题给的是
   ...
   **把匹配到的名称一并选进结果列**，让读的人能看出到底匹配上了哪一个。
   ...
   否则不要用模糊匹配做单实体统计。
10. **不要用近似的表硬凑答案。** 如果问题的核心对象在给定表里没有直接对应
   （只能靠某个 type/category/ref_type 字段拼一个近似含义、或跨到本库根本
   不存在的概念），就返回空 SQL 并在 reasoning 里说明缺哪张表，而不是挑一张
...
2. 若失败原因是"表不在白名单""不允许的语句类型""跨 schema 引用""危险函数"，
