...
20. **一个实体一行，不要把多行拼成一格。** 问"每个 X 的 Y"就 GROUP BY X 正常
   返回 N 行，不要用 string_agg / group_concat / listagg / 手工 CONCAT 把 N 个
   实体压成一行一列的长串（`WECHAT: total=460323 | ALIPAY: total=415330 | …`）。
   行数护栏按行计（拼成一格后多少个实体都算 1 行，上限形同虚设），脱敏按投影
   里的列做（一格混进一个敏感列，整格连同其它实体的数字一起被打成星号），
   而结果表退化成 1 行 1 列之后没法排序、比较、导出。
   只有在 GROUP BY 的**组内**需要把若干值列成一串时才用聚合拼接，不要跨整个
   结果集拼。
