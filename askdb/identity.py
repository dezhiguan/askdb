...
# 现在整套模型只剩三条，别再往回加第四条：
#   1. 所有角色的可见面**完全相同**（含匿名）。
#   2. 角色差别只在**处置权**上，一共两位：APPROVE（审批与复核）只给系统管理员，
#      OPS_RESOLVE（执行期故障处置）给运维与系统管理员。2026-09-11 之前这里
#      写的是「唯一的差别是 APPROVE」—— 加 OPS_RESOLVE 没有推翻那句话的用意：
#      可见面仍然完全相同，多出来的两位都只决定**能不能把一条卡住的任务往前推**，
#      一行数据也不会因此多看到。往这里加第三位之前，先问它是不是同一性质。
...
ROLES: tuple[Role, ...] = (
    ...
    # 2026-09-11 新增。**它不是「运营」的另一个写法** —— OPERATIONS（运营）看的是
    # 经营数据，SRE（运维）管的是这套系统本身跑不跑得起来。两个中文名只差一个字，
    # 所以判据写在这里：进 SRE 的人负责数据源、连接与执行期故障，不负责业务口径。
    #
    # 加它的理由是「等待运维」这一档此前只是个标签：审计会把执行期故障（EXEC）
    # 判成 needs_operator，界面上写着「等运维」，而系统里根本没有一个叫运维的角色，
    # 更没有任何人能把它标成已处理。于是那一档只进不出。
    Role("SRE", "运维", "平台可见面 + 故障处理",
         "处理数据源连接与执行期故障。可见面与其他角色相同；"
         "多出来的只有「把一条执行期故障标为已处理」这一项。"),
    Role("SYS_ADMIN", "系统管理员", "平台可见面 + 审批 + 故障处理",
         "管理角色成员，审批高成本查询与数据源变更，并复核存疑结果。"
         "**权限最大的角色**：审批、复核、成员增删，以及运维的故障处理。",
         system=True),
)
...
APPROVE = "approve"                 # Q-08 / S-03~05 审批放行（事前）与结果复核（事后）
#: 把一条执行期故障标为已处理。**与 APPROVE 分开的理由**：审批判断的是
#: 「该不该让它去跑」，这里判断的是「库恢复了没有」—— 后者是运维事实，
#: 不需要、也不应该要一个能批钱的人来点。合成一个位就等于说运维=审批。
OPS_RESOLVE = "ops.resolve"         # 执行期故障处置
...
#: 而这一层配错等于开门。
#:
#: 结构就是下面这四行，别再长回一张按角色分档的表：
#:   · 所有登录角色 = _READ | _WRITE，**完全相同**
#:   · SRE 额外多 OPS_RESOLVE
#:   · SYS_ADMIN 额外多 APPROVE、MEMBERS_WRITE 与 OPS_RESOLVE
...
CAPABILITIES: dict[str, frozenset[str]] = {
    ...
    "OTHER": _READ | _WRITE,
    # 运维多的那一位只管故障处置，**不含 APPROVE** —— 能重启库的人不等于
    # 能放行一次全表扫描，这两件事的代价完全不同。
    "SRE": _READ | _WRITE | {OPS_RESOLVE},
    # 系统管理员含 OPS_RESOLVE：线上很长一段时间只有他一个人在，
    # 「等运维」不该因为运维角色还没招到人就永远没人能动。
    "SYS_ADMIN": _READ | _WRITE | {APPROVE, MEMBERS_WRITE, OPS_RESOLVE},
    ANONYMOUS: _READ,
}
