...
GENERAL_RULES: list[str] = [
    ...
    "枚举先查证，拿不准就先 SELECT DISTINCT：按某列的枚举值过滤前，先确认该列的确切"
    "取值。get_table_schema 若没给出该列的枚举，**必须先用 execute_sql 跑一条 "
    "SELECT DISTINCT <列> FROM <表>（可带 LIMIT）看真实取值，再据此写过滤**，绝不靠"
    "列名或常识猜取值（例：判「JD 文档」应先看是 chunk_type 还是 file_type 承载业务"
    "类型、其取值是不是 'JD'；判「已完成」应先看 parse_status 的真实取值是 "
    "'COMPLETED' 还是 'SUCCESS' 等，不要凭空假设）。",
