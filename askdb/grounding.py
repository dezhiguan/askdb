...
#: 逗号只有在"三位一组"时才是千分位。写成 [\d,]* 会把 SQL 里的
#: `NULLIF(782738,0)` 读成 782738,0 → 7,827,380 —— 模型在结论里引用自己写的
#: SQL 时就会踩到（2026-09-12 第四轮 R4，答案本身完全正确却被点名）。
_NUM = re.compile(r"(?<![0-9A-Za-z_.\-])(?:\d{1,3}(?:,\d{3})+|\d+)(?:\.\d+)?")
