...
- **审计、审批、复核、检查点、回归成绩、每日配额全部存在 PostgreSQL**
  ...
  **切换当天必须跑一次导入**（先发版，再导；发版后没有任何进程再写文件，
  ...
  读取侧 2026-09-09 同日做了下推（`audit.AuditFilter` → `auditstore._clauses`）：
  分页、计数、下拉取值各自只带回自己那点数据，必须整遍的走服务端游标。
  **换库本身不解决内存** —— 首版 `read_audit` 是无 LIMIT 的全表读再 fetchall
  全量缓冲，等于把同一个 OOM 从文件搬到库上。
